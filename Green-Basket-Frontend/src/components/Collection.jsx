...
theme: "dark",
      ...
      const arr = allCollection.map(item=>item._id === pdt._id?{...item,quantity:item.quantity+1}:item);
      setallCollection(arr);
...
theme: "dark",
      ...
      const arr = allCollection.map(item=>item._id === pdt._id && item.quantity>0?{...item,quantity:item.quantity-1}:item);
      setallCollection(arr);
...
<div className="filters md:m-20 flex gap-5 text-xl font-serif">
    ...
    <select name="price" id="" value={SortbyPrice} className='bordergreen' onChange={(e)=>setSortbyPrice(e.target.value)}>
     ...
     <div className='flex flex-wrap justify-center md:justify-start container gap-5 md:gap-10 md:m-10 m-5 md:mx-20'>
      

      {allCollection.map(item=>(
        <div key={item._id} className='w-36 text-[20px] hover:scale-105 transition-transform rounded-3xl shadow-lg shadow-green-900 flex flex-col items-center textgreen'>
